...
\begin{enumerate}
  ...
  \item Derive solar direction, eclipse/penumbra exposure, and instantaneous solar flux.
...
one formula-focused module, not inline in UI code. Any symbol in the equations below
that does not carry a frame index $k$ and is not a direct configuration input is treated
as one of these fixed pass-level parameters.
...
\midrule
\endhead
$G_{elem}, q_{scan}, \Delta \phi_{LSB}, k_{\phi,T}, k^{arr}_T, T^{arr}_{ref}, d_{elem}, \kappa_{bw}$ & Array gain, scan-loss, thermally induced phase error, thermal gain degradation, spacing, and beamwidth constants \\
$\theta_{FOR}, \Delta G^{sat,tx}, \Delta G^{sat,rx}$ & Maximum electronic field of regard and satellite TX/RX calibration offsets \\
$k_{NF}, L_{filter}, L_{conv}, L_{impl}, T^{rf}_{ref}, \Delta NF^{sat,rx}$ & RF front-end thermal sensitivity, fixed losses, RF reference temperature, and satellite RX noise-figure offset \\
$k_{comp}, P_{1dB}, P_{sat}, k^{pa}_T, k^{pa}_{\eta}, T^{pa}_{ref}, T^{pa}_{derate}, \eta^{pa}_0, \eta^{pa}_{min}, \eta^{pa}_{max}$ & PA compression, saturation, thermal derating, and efficiency parameters \\
$\alpha_f, \dot{f}_{drift}, \sigma_{t,0}, k_t, T^{osc}_{ref}$ & Oscillator thermal drift, frequency drift rate, jitter coefficients, and reference temperature \\
$C_i, R_i, A_i, \alpha_i, T_{sink}, T^i_{th}, \Delta T^i_{tag}$ & Thermal capacitance, thermal resistance, exposed area, solar-absorption coefficients, sink temperature, and explainability thresholds for subsystem $i$ \\
$\Phi_{sun,1AU}, R_{\odot}, A_{sol}, \eta_{sol}, E_{batt,\max}, P_{batt,\max}, SOC_{min}, k^{bus}_P$ & Solar constant at 1 AU, Sun radius, solar-array parameters, battery parameters, minimum battery reserve, and bus-shedding gain \\
$P^{beam}_{0}, k^{arr}_{scan}, P^{rf}_{0}, k^{rf}_{cpu}, P^{osc}_{0}, P^{house}_{0}, P^{comp}_{idle}, P^{comp}_{max}$ & Baseline array, RF, oscillator, housekeeping, and compute power draws \\
...
$P^{gs}_{tx,nom}, P^{gs}_{tx,\min}, P^{gs}_{tx,\max}, L^{gs}_{tx,rf}, NF^{gs}_{nom}, T^{ant,gs}_{0}, T^{ant,sat}_{0}$ & Ground-station nominal uplink power, power limits, fixed TX RF loss, nominal RX noise figure, and baseline ground/satellite antenna temperatures \\
...
$L^{zen,dl}, L^{zen,ul}, E_{floor}, L^{misc,dl}, L^{misc,ul}, k^{dl}_{cfo}, k^{ul}_{cfo}, k^{dl}_{pa,evm}, k^{ul}_{pa,evm}, R^{dl}_{s}, R^{ul}_{s}$ & Atmospheric-loss, link-margin, residual-frequency, PA-EVM, and symbol-rate parameters for both directions \\
...
$L^{array}_{tag}, L^{comp}_{tag}, \tau^{deadline}, \Delta f^{dl}_{tol}, \Delta f^{ul}_{tol}$ & Explainability-tag thresholds for array degradation, PA compression, scheduling delay, and residual frequency error \\
\bottomrule
\end{longtable}
...
\subsection{Solar Ephemeris, Flux, and Eclipse Factor}

Let $JD_k$ be the Julian date at time $t_k$, and let
\begin{equation}
  T_k = \frac{JD_k - 2451545.0}{36525}.
\end{equation}

Using a standard low-order solar ephemeris, define the mean longitude, mean anomaly,
ecliptic longitude, and obliquity:
\begin{align}
  L_k &= \operatorname{mod}\left(280.46 + 36000.771 T_k, 360^\circ \right), \\
  g_k &= \operatorname{mod}\left(357.528 + 35999.05 T_k, 360^\circ \right), \\
  \lambda^{\odot}_k
  &= L_k + 1.915 \sin g_k + 0.020 \sin 2 g_k, \\
  \varepsilon_k &= 23.4393^\circ - 0.0130^\circ T_k.
\end{align}
Angles in this subsection are converted to radians before evaluation in trigonometric
functions.

The Earth-to-Sun unit vector in ECI is then
\begin{equation}
  \hat{s}^{ECI}_k
  =
  \begin{bmatrix}
    \cos \lambda^{\odot}_k \\
    \cos \varepsilon_k \sin \lambda^{\odot}_k \\
    \sin \varepsilon_k \sin \lambda^{\odot}_k
  \end{bmatrix}.
\end{equation}

Let the Earth-Sun distance in astronomical units be
\begin{equation}
  d^{\odot}_{AU,k}
  =
  1.00014
...
The instantaneous solar flux is
\begin{equation}
  \Phi_{sun,k} = \frac{\Phi_{sun,1AU}}{(d^{\odot}_{AU,k})^2}.
\end{equation}

If an Earth-fixed solar direction is needed,
\begin{equation}
  \hat{s}^{ECEF}_k = R_3(\theta_{g,k}) \hat{s}^{ECI}_k.
\end{equation}

Define the satellite-to-Sun vector and the Earth-center look direction from the
satellite as
\begin{align}
  \vect{\rho}^{\odot}_k
  &= d^{\odot}_{AU,k} \cdot \mathrm{AU} \cdot \hat{s}^{ECI}_k - \vect{r}^{ECI}_k, \\
  \hat{e}_k
  &= - \frac{\vect{r}^{ECI}_k}{\norm{\vect{r}^{ECI}_k}}, \\
  \hat{u}^{\odot}_k
  &= \frac{\vect{\rho}^{\odot}_k}{\norm{\vect{\rho}^{\odot}_k}}.
\end{align}

The apparent angular separation between the Earth disk and solar disk is
\begin{equation}
  \psi_k = \arccos \left( \hat{e}_k \cdot \hat{u}^{\odot}_k \right).
\end{equation}

The apparent angular radii of Earth and Sun from the satellite are
\begin{align}
  \theta^{\oplus}_k
  &= \arcsin \left( \frac{R_{\oplus}}{\norm{\vect{r}^{ECI}_k}} \right), \\
  \theta^{\odot}_k
  &= \arcsin \left( \frac{R_{\odot}}{\norm{\vect{\rho}^{\odot}_k}} \right).
\end{align}

Use the following first-order penumbra model for solar visibility:
\begin{equation}
  S_k
  =
  \clip\left(
    \frac{\psi_k - (\theta^{\oplus}_k - \theta^{\odot}_k)}{2 \theta^{\odot}_k},
    0,
    1
  \right).
\end{equation}

Thus $S_k = 0$ in umbra, $S_k = 1$ in full sun, and intermediate values represent
partial solar-disk visibility in penumbra. For the short pass window, assume the
solar array can track the Sun closely whenever it is visible, so orbital modulation of
generated solar power enters through $S_k$ and $\Phi_{sun,k}$.
...
\begin{equation}
  \sigma_{\phi,therm,k}
  =
  k_{\phi,T}\max(0, T^{arr}_k - T^{arr}_{ref}).
\end{equation}
\begin{align}
  \sigma^2_{\phi,k}
...
Use a first-order thermal gain penalty for the array:
\begin{equation}
  L^{arr}_{therm,k}
  =
  k^{arr}_T \max(0, T^{arr}_k - T^{arr}_{ref}).
\end{equation}

For the baseline implementation, assume the electronically steered array introduces
no residual closed-loop pointing error beyond the commanded steering solution:
\begin{equation}
  \theta^{arr}_{point,k} = 0,
  \qquad
  L^{arr}_{point,k} = 0.
\end{equation}
...
\begin{equation}
  Q^i_{sun,k} = \alpha_i A_i \Phi_{sun,k} S_k,
\end{equation}
where $S_k$ is the sun-exposure factor derived from the orbital solar-visibility model.

\subsection{Solar, Battery, and Bus Budget}

Solar generation is
\begin{equation}
  P^{sol}_k = \eta_{sol} A_{sol} \Phi_{sun,k} S_k.
\end{equation}

For the baseline report, no active heater or pumped thermal-control actuator model is
included, so the explicit thermal-control electrical draw is
\begin{equation}
  P^{therm}_k = 0.
\end{equation}
...
Because the current scenario configures RHCP on both the satellite and ground terminal,
use a common unit polarization basis $\hat{e}_{RHCP}$ and take
\begin{equation}
  \hat{e}^{sat,tx}_k
  =
  \hat{e}^{sat,rx}_k
  ...
  \hat{e}_{RHCP}.
\end{equation}
The general inner-product form is retained so future scenarios can introduce
cross-polarization mismatch explicitly.
...
For the baseline pass, hold the effective antenna noise temperatures constant:
\begin{equation}
  T^{ant,gs}_k = T^{ant,gs}_{0},
  \qquad
  T^{ant,sat}_k = T^{ant,sat}_{0}.
\end{equation}
...
\begin{enumerate}
  ...
  \item \texttt{simulation/sun}: solar ephemeris, Earth shadowing, penumbra exposure, solar flux.
